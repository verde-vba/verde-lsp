...
## Sprint N+38 (2026-04-21)

### Sprint Goal
PBI-35: `textDocument/signatureHelp` 実装 — カーソルが関数呼び出しの引数リスト内にある場合に関数シグネチャと `active_parameter` インデックスを返す。Phase 1 の先頭 PBI。

### Path Chosen
後方テキストスキャン方式: カーソルオフセットから後方に文字をスキャンし、括弧ネスト深度を追跡して `(` を探す。AST に依存しないため構文エラー中のコードでも動作する。

### Scope
- `src/signature_help.rs` 新規作成: `find_call_context()` (後方スキャン) + `build_signature_info()` + `format_param()`
- `src/lib.rs`: `pub mod signature_help;` 追加
- `src/server.rs`: `signature_help` ハンドラ + `signature_help_provider` を `server_capabilities()` に追加
- `tests/signature_help.rs` 新規作成: 3 テスト (基本シグネチャ / active_parameter / cross-module)

### Acceptance Criteria
全 AC を達成:
1. `Sub Foo(x As Integer)` で `Foo(|)` → label `"Sub Foo(x As Integer)"`
2. `Foo(1, |2)` → `active_parameter == 1`
3. cross-module Public 関数でもシグネチャ返却
4. `server_capabilities()` に `signature_help_provider` が追加された
5. 111 → 114 green、clippy 0、fmt pass

### Result
114 green (lib 47 / integration 67) / clippy 0 / fmt pass。
- `signature_help.rs`: 156 行。`find_call_context` が `O(cursor)` の後方スキャンで関数名と active_parameter を返す
- 括弧ネスト `Foo(Bar(|))` で `Bar` が返る設計 (depth tracking)
- cross-module フォールバックは既存 `find_public_symbol_in_other_files` を再利用

---

## Sprint N+38 レトロスペクティブ (2026-04-21)

### Sprint Goal 達成状況

目標「PBI-35 signatureHelp」を完全達成。Phase 1 の最初の機能 PBI を 1 Sprint で消化。

### KPT

#### Keep
- **後方スキャン方式の選択**: AST を使わずソーステキストを直接スキャンすることで、`(` の入力直後という「まだ AST が正しく parse できない状態」でも動作する。LSP の signatureHelp は入力中に呼ばれることが前提なので、この方針は正しい
- **`hover.rs` の `format_params` パターン再利用**: hover.rs で確立した `ParameterInfo → "x As Integer"` 変換を signature_help.rs でも同じロジックで実装。コードベースのフォーマット一貫性を維持
- **`clamped` で active_parameter を上限クリップ**: VBA は variadic args (`ParamArray`) を持てるため、渡された引数数がパラメータ数を超える場合にクラッシュしない設計

#### Problem
- `find_call_context` は ASCII バイト操作のみ。VBA コード中の UTF-8 文字 (コメントや文字列リテラル内の日本語など) が `(` や `,` と誤マッチする可能性はないが、マルチバイト文字列内の `(`/`,` バイトが ASCII と一致しないため実際には問題なし (UTF-8 では ASCII バイトは ASCII のみ)
- Named arguments (`Foo x:=1, y:=2`) は未対応。`:=` のカンマカウントが不正になる可能性があるが VBA コードでの使用頻度は低い

#### Try
- PBI-36 (workspace/symbol) または PBI-37 (documentHighlight XS) を次 Sprint に選択
- signatureHelp に `None` が返るケース (カーソルが呼び出しの外) のテストを追加してエッジケースを固定するか検討 (現在は確認テストなし)

### Next Sprint Candidates

| 優先度 | PBI | 理由 |
|---|---|---|
| 1 | PBI-37 | documentHighlight (XS)。同名シンボルハイライト。最小工数でユーザー体験向上 |
| 2 | PBI-36 | workspace/symbol (S)。横断検索。中規模 |
| 3 | PBI-38 | foldingRange (S)。Sub/Function ブロック折りたたみ |

---

## Sprint N+39 (2026-04-21)

### Sprint Goal
PBI-37: `textDocument/documentHighlight` — 現在ファイル内の同名シンボルを全てハイライトして返す。XS (既存の `find_all_word_occurrences` + `text_range_to_lsp_range` を再利用)。

### Path Chosen
`references.rs` のシングルファイル版として実装。`all_file_sources()` を使わず `with_source()` のみで完結。

### Scope
- `src/document_highlight.rs` 新規作成 (29 行)
- `src/lib.rs`: `pub mod document_highlight;` 追加
- `src/server.rs`: `document_highlight` ハンドラ + `document_highlight_provider` 追加
- `tests/document_highlight.rs` 新規作成: 2 テスト

### Result
116 green (lib 47 / integration 69) / clippy 0 / fmt pass。1 commit。

---

## Sprint N+39 レトロスペクティブ (2026-04-21)

### Sprint Goal 達成状況

目標「PBI-37 documentHighlight」を完全達成。XS 見積通り、実装 29 行。

### KPT

#### Keep
- **XS PBI を 1 Sprint で消化するテンポ**: 実装 + テスト + server.rs 更新を 30 分以内に完了。小さい PBI の連打で Phase 1 の進捗が見えやすい
- **`find_word_at_position` の動作を理解したテスト修正**: 最初のテストで `{0,3}` (空白位置) が実は "Sub" を返すと発見し、テスト前提を修正。`find_word_at_position` は「隣接するワード」を返す設計を再確認

#### Problem
- `document_highlight` は現在ファイルのみ対応。LSP 仕様では cross-file highlight も定義されているが VBA では通常同一ファイル内で充分
- `DocumentHighlightKind::TEXT` のみで `READ`/`WRITE` 区別は未実装

#### Try
- PBI-36 (workspace/symbol, S) または PBI-38 (foldingRange, S) を次 Sprint に
- plan.md が 1000 行超になっているため、次 Sprint 完了時に compact を検討

### Next Sprint Candidates

| 優先度 | PBI | 理由 |
|---|---|---|
| 1 | PBI-36 | workspace/symbol (S)。プロジェクト横断シンボル検索 |
| 2 | PBI-38 | foldingRange (S)。Sub/Function ブロック折りたたみ |
| 3 | PBI-39 | codeAction — "Dim を追加" quick fix (M) |

---

## PBI-35 リファインメント (2026-04-21)

### 概要

`textDocument/signatureHelp` を実装する。カーソルが関数呼び出しの引数リスト内にある場合に、その関数のシグネチャとアクティブパラメータインデックスを返す。
...
| cursor 位置 → procedure 特定 | `proc_ranges` + `position_to_offset` の確立済みパターン (completion/hover/definition/rename で共通使用) |
| AST Statement | `StatementNode` 各バリアントは `tokens: Vec<SpannedToken>` を保持 — raw トークン列から関数名と `(` をスキャンする手法が使える |
| `server.rs` | `server_capabilities()` に `signature_help_provider` を追加する必要あり |
| 新規ファイル | `src/signature_help.rs` を新規作成 |

### MVP スコープ

- 対象: `Foo(x, |y)` 形式 — カーソルが `(` の右側にある場合にシグネチャを返す
- trigger characters: `(` と `,`
- cross-module 関数: 対象 (symbols テーブルで既に解決可能)
- Named arguments (`Foo x:=1`): **対象外** (MVP スコープ外)
- VBA ビルトイン関数のシグネチャ: **対象外** (MVP スコープ外、将来 vba_builtins.rs 拡張で対応)

### 実装ステップ (TDD + Tidy First)

#### Step 1: Tidy First — 構造変更なし

`SymbolDetail::Procedure.params` と `ParameterInfo` が既に完全な情報を持つため、Tidy First の事前抽出は不要。

#### Step 2 (RED): 基本シグネチャ返却テスト

`tests/signature_help.rs` 新規作成。単一パラメータ関数 `Sub Foo(x As Integer)` で `Foo(|)` にカーソルを置き、シグネチャラベルが `"Sub Foo(x As Integer)"` として返ることを確認するテストを追加 (1 テスト)。

#### Step 3 (GREEN): 最小実装

`src/signature_help.rs` を新規作成し、以下のロジックを実装:
1. カーソルオフセットを起点にソーステキストを後方スキャンし、対応する `(` を探す
2. `(` の直前にある identifier を関数名として取得
3. 現在ファイルの symbols から関数を検索 (見つからなければ cross-module フォールバック)
4. `SymbolDetail::Procedure { params, .. }` からシグネチャラベルとパラメータラベルを構築
5. `server.rs` に `signature_help` ハンドラを追加、`server_capabilities()` に `signature_help_provider` を追加

#### Step 4 (RED): `active_parameter` テスト

`Foo(x, |y)` の形でカーソルが2番目の引数にある場合に `active_parameter == 1` が返ることを確認するテスト (1 テスト)。

#### Step 5 (GREEN): `active_parameter` 実装

カーソルオフセットから `(` までの間にある `,` の数を数えてインデックスを決定。

#### Step 6 (RED): cross-module 関数テスト

別ファイルで定義された Public 関数のシグネチャが返ることを確認するテスト (1 テスト)。

#### Step 7 (GREEN): cross-module フォールバック

`find_public_symbol_in_other_files` を使用して cross-module 関数を解決。

### Acceptance Criteria

1. `Sub Foo(x As Integer)` で `Foo(|)` にカーソルを置くと `SignatureInformation.label == "Sub Foo(x As Integer)"` が返る
2. `Sub Foo(x As Integer, y As String)` で `Foo(1, |)` にカーソルを置くと `active_parameter == 1` が返る
3. `Function Bar(n As Long) As String` で `Bar(|)` にカーソルを置くと `"Function Bar(n As Long) As String"` が返る
4. 関数呼び出しの外 (例: 行頭 `Sub` キーワードの直後) では `None` が返る
5. cross-module Public 関数に対してもシグネチャが返る
6. 111 → 114 green (+3 テスト増分見積)、clippy -D warnings 0 件、fmt pass
7. `server_capabilities()` に `signature_help_provider: Some(SignatureHelpOptions { trigger_characters: Some(vec!["(", ","]), .. })` が追加されている

### 実装ファイル

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `src/signature_help.rs` | 新規作成 | `signature_help(host, uri, position) -> Option<SignatureHelp>` |
| `src/server.rs` | 変更 | `signature_help` ハンドラ追加、`server_capabilities()` 更新 |
| `src/lib.rs` | 変更 | `pub mod signature_help;` 追加 |
| `tests/signature_help.rs` | 新規作成 | +3 テスト (基本シグネチャ / active_parameter / cross-module) |

### 設計上の判断と制約

- **後方スキャン方式**: カーソル位置からテキストを後方スキャンして `(` と関数名を探す方法を採用。AST の `StatementNode` はトークン列を保持しているが、どの Statement がカーソルを含むかを特定するより、ソーステキストの直接スキャンの方が実装が単純でテストが書きやすい
- **括弧ネスト対応**: `Foo(Bar(|), x)` のようなネストされた呼び出しでは、最も内側の呼び出し (`Bar`) のシグネチャを返すため、後方スキャン時に括弧のネスト深度を追跡する
- **エラー耐性**: 構文エラーのある不完全なコード (入力中) でも動作する必要がある。AST parse に依存せずテキストスキャンで解決する理由の一つ
...
### Result
109 green (lib 47 / integration 62) / clippy 0 / fmt pass。
- plan.md: PBI-32 Cancel 化 + PBI-32b / PBI-32c 新設 (docs-only commit)
- `src/server.rs`: `server_capabilities()` を pure function に抽出 (Tidy First) → RED test → GREEN で `position_encoding: Some(PositionEncodingKind::UTF16)` 追加
- 4 commit 構成: planning docs / refactor (structural) / test RED / feat GREEN

### PBI-32b は次 Sprint 送り
RwLock poison 経路は現状到達不可能 (単一プロセス・単一 runtime) かつ clippy も警告していないため、PBI-32b は urgency 低。Windows 配布前の Phase 0 残項目としては PBI-33 (Windows CI) の方が実害に近い。

---

## Sprint N+34 レトロスペクティブ (2026-04-21)

### Sprint Goal 達成状況

目標「PBI-32 前提調査 + PBI-32c positionEncoding 明示」を完全達成。

### KPT

#### Keep
- **調査 Sprint を独立させた価値**: rg 一発で 18 件の `.expect()/panic!()` が全て `#[cfg(test)]` 内と判明。PBI-32 を盲目的に着手していたら「panic 経路がない箇所で panic 除去」という無意味な作業になっていた。調査 → 計画修正 → 必要なら実装、という順序が機能した
- **Tidy First を律儀に守った**: `server_capabilities()` 抽出 → RED → GREEN の 3 commit 分離。抽出なしで RED を書こうとすると `LanguageServer::initialize` の Client 依存テストを書く羽目になり、テスト構造が肥大化していた。refactor が test-ability を上げた好例
- **plan.md の誤った cite を恥ずかしがらずに訂正**: 元の PBI-32 cite (`parse.rs:860, :1426`) はどちらも test code を指していた。これを Cancel として記録に残し、代わりに実在する 2 次 panic 経路 (RwLock) を PBI-32b として切り出したことで、roadmap の信頼性が維持される

#### Problem
- `LanguageServer` trait 実装 (async fn initialize) 自体は Client mock なしでは直接テストできない。今回は pure function 抽出で回避したが、将来 `did_open` / `did_change` などのハンドラをテストしたい場合は tower-lsp の test harness か Client モックが必要
- plan.md のバックログ表に PBI 番号の suffix (PBI-32b, PBI-32c) が登場し始めた。今後も派生 PBI が増えると番号管理が複雑化する懸念

#### Try
- PBI-33 (Windows CI) を次 Sprint 候補筆頭に置く。`windows-latest` matrix + `file:///C:/...` URI 経路の smoke test。Verde desktop Windows 配布の前提条件
- PBI-32b (RwLock poison) は **defensive 強化** であって bug fix ではないため、PBI-33 / PBI-34 より後回しでよい。`std::sync::RwLock` → `tokio::sync::RwLock` の置換は poison 概念そのものを消せる (tokio の RwLock に poison はない) ため別 Sprint で検討
- `positionEncoding` 明示宣言したことで、クライアント側が別 encoding を要求するケースの実機テスト (Verde desktop での stdio probe) が次の検証ポイント
